...
| `source-material/conversational-design-hall-summary.md` | Comprehensive summary of _Conversational Design_ (Erika Hall) — how human conversation actually works (Grice's maxims, the eight conversational principles, personality/voice, key-moments framework) applied to any interface. Directly relevant to the end-user chat-feel problem. **Not yet folded into the system-prompt doctrine** — load it into context when working on chat-experience or voice/personality decisions. |

> **On the book summaries:** the doctrine is already baked into `kodara-cpo-system-prompt.md`, so you don't need to load the book-summary files at runtime for the agent to behave correctly — they're kept in the repo as the canonical reference for *why* it reasons the way it does, and so the doctrine can be audited or updated over time. Load them into context only when you want the agent to cite chapter-level detail or you're revising the system prompt itself.
